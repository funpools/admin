...
async function getPool(poolID) {
  ...
  if (poolData.parentPool != null) {
    ...
    return {
      ...
      requiresPermission:
        poolData.requiresPermission != null
          ? poolData.requiresPermission
          : true,
      admins: poolData.admins ? poolData.admins : [],
      unsubscribed: poolData.unsubscribed ? poolData.unsubscribed : [],
      sentPoolClosingNotification: poolData.sentPoolClosingNotification
        ? poolData.sentPoolClosingNotification
        : false,
    };
  } else {
    return {
...
exports.poolUpdate = functions
  ...
  .onWrite(async (change, context) => {
    ...
    let functionLog =
      "Pool " +
      context.params.poolID +
      " with state: " +
      newData.state +
      " has been updated! "; //This is a log so we can keep track of the pool and only consle. Log once for quota reasons
      ...
      try {
        ...
        if (pool.childPools != null) {
          pool.childPools.forEach(function (cpoolID) {
            poolPromises.push(
              gradePool(cpoolID, messageType).then(function (result) {
        ...
        function gradeUser(user) {
          ...
          if (userDoc != null && userDoc.get("answers") != null) {
            ...
            for (let i = 0; i < pool.questions.length; i++) {
              // If this question is answered and the user answered correctly
              if (
                answers[pool.questions[i].id] != null &&
            ...
            for (let i = 0; i < pool.tiebreakers.length; i++) {
              ...
              tieBreakerScores.push(
                Math.abs(
                  answers[pool.tiebreakers[i].id] - pool.tiebreakers[i].answer
                )
              );
            }
          } else {
        ...
        if (pool.users != null) {
          //If there are any users in this pool
          ...
          if (
            pool.questions.every((a) => a.correctAnswer != null) &&
            pool.tiebreakers.every((e) => e.answer != null)
          ) {
            ...
            for (let i = 0; i < gradedUsers.length; i++) {
              ...
              if (previousUser != null && previousUser.score == user.score) {
                for (let x = 0; x < pool.tiebreakers.length; x++) {
                  let tieScore =
                    user.tieBreakerScores[x] - previousUser.tieBreakerScores[x];
                  poolLog.log(
                    "resolving tie for user placing score is: " + tieScore
                  );
                  ...
                  if (tieScore != 0) {
                    if (tieScore > 0) {
                      ...
                      poolLog.log(
                        "THIS is unlikely to happen. In fact it may be impossible"
                      );
          ...
          try {
            poolLog.log(
              "Sending notifications to users. Notification type: ",
              messageType
            );
                  ...
                  switch (messageType) {
                    case "active":
                      sendNotification(user.uid, {
                        ...
                        text:
                          pool.name +
                          " is now active. Click here to view the pool! ",
                        type: "PU",
                      });
                      break;
                    case "closed":
                      sendNotification(user.uid, {
                        ...
                        text:
                          "The " +
                          pool.name +
                          " is closed. Your ranking is " +
                          user.place +
                          "/" +
                          pool.users.length +
                          ". Check here to see your results!",
                        type: "PU",
                      });
                      break;
                    case "score-update":
                      sendNotification(user.uid, {
                        ...
                        text:
                          "A question in " +
                          pool.name +
                          " has been updated—check here for your score!",
                        type: "PU",
                      });
                      break;
                    default:
        ...
        return "poolLog.getLog()"; //TODO once we migrate to type script we can fix this permanently
      } catch (error) {
        poolLog.log("ERROR");
        // poolLog.toConsole();
        console.error(error);
    ...
    if (previousData.state == null || previousData.state != newData.state) {
      console.log(
        "The state has changed for pool: " +
          context.params.poolID +
          " old: " +
          previousData.state +
          " new: " +
      ...
      switch (newData.state) {
        ...
        case "closed":
            ...
            if (user.isWinner) {
              ...
              await admin
                ...
                .then(async (userRecord) => {
                  ...
                  winnersList =
                    winnersList +
                    '<p style="font-size: large"></br>Name:' +
                    userData.firstName +
                    " " +
                    userData.lastName +
                    " <br /> UID: " +
          ...
          await admin
            ...
            .add({
              to: [
                "tsmith@funsportspools.com",
                "development@funsportspools.com",
                "admin@funsportspools.com",
                "Haley.Sacotte@wyecomm.com",
              ], //'tsmith@funsportspools.com', 'development@funsportspools.com', 'admin@funsportspools.com,mike@onairsportsmarketing.com,Haley.Sacotte@wyecomm.com'
              message: {
                subject: "Winner info for " + poolData.name,
                html:
                  '<div style="margin: 32px auto; padding: 32px; max-width: 500px; background-color: #f0f0f0; border-radius: 8px">\
              <img src="https://admin.funpools.app/logo.png" style="width: 40%; max-width: 150px; display: block; margin: 0 auto;"/>\
            <p style="font-size: large"></br>Hi Admins,<br />' +
                  poolData.name +
                  " has been closed and the winner info is: </br></p>" +
                  winnersList +
                  "</div>",
              },
            })
        ...
        case "open":
          ...
          if (previousData.state == null || previousData.state === "draft") {
            console.log(
              "Pool: " +
                newData +
                " has been opened! is dev: " +
                newData.tags.includes("dev")
            );
            try {
              ...
              await sendAnnouncementNotification(
                ...
                "The " +
                  newData.name +
                  " pool has been opened—tap to play now!",
                "/pool/?id=" + context.params.poolID,
                "open-" + context.params.poolID,
                newData.tags.includes("dev")
              );
            } catch (error) {
      ...
      switch (newData.state) {
        case "active":
          ...
          if (
            JSON.stringify(newData.questions) !=
              JSON.stringify(previousData.questions) ||
            JSON.stringify(newData.tiebreakers) !=
              JSON.stringify(previousData.tiebreakers)
          ) {
            //If the questions/tiebreakers have changed
            // Grade the pool and send users a score update
            let results = await gradePool(
              context.params.poolID,
              "score-update"
            );
            console.log(
    ...
    console.log(
      "Finished execution of pool update for: " + context.params.poolID
    );
...
exports.poolClosing = functions.pubsub
  ...
  .onRun(async function (context) {
          ...
          pool.users.forEach((user, i) => {
            userPromises.push(
              sendNotification(user.uid, {
                ...
                text:
                  "Enter your answers now: " + pool.name + " is closing soon!",
                type: "PU",
              }).then((result) => {})
            );
          });
          console.log(
            "Sent closing notificatoin to " + pool.users.length + " users!"
          );
        } else {
    ...
    for (var i = 0; i < querySnapshot.docs.length; i++) {
      ...
      notificationPromises.push(
        sendClosingNotificatoin(querySnapshot.docs[i].id)
      );
    }
...
exports.weeklyPool = functions.pubsub
  .schedule("0 0 * * *")
  .onRun(async function (context) {
    //g2DfdYxi9z
    //every monday 00:00
    ...
    console.log(
      querySnapshot.size,
      " pools are closing this week. Start and end date:",
      startDate,
      endDate
    );
      ...
      if (doc.exists && doc.data().state == "open") {
        return db
          ...
          .add({
            ...
            requiresPermission:
              data.requiresPermission != null ? data.requiresPermission : true,
            private: true,
          })
          .then(function (doc) {
...
exports.joinPool = functions.https.onCall(async function (data, context) {
  //Function for joining and leaving pools
  /**
  ...
  if (data.uid == null || data.poolID == null || data.join == null) {
    ...
    throw new functions.https.HttpsError(
      "failed-precondition",
      "The function was called with invaid data ",
      data
    );
  }
  ...
  if (pool != "invalid") {
    if (pool.state != "closed" && !(pool.state == "active" && join)) {
      //If the pool is not closed and (The the user is not trying to join an active pool)
      if (userID == targetUid) {
        //If the user is requesting this operation as themselves

        console.log("Loaded pool data: ", JSON.stringify(pool));

        if (join) {
          //If the user wants to join the pool

          if (
            !pool.private ||
            pool.allowedUsers.includes(userID) ||
            !pool.requiresPermission
          ) {
            ...
            //This user is not allowed to join this pool so request permission from the pool admin/captian
            ...
            await sendNotification(adminT, {
              ...
              title:
                ...
                " " +
                targetUser.lastName +
                " has requested to join your pool.",
              text:
                targetUser.firstName +
                " " +
                targetUser.lastName +
          ...
          //The user does not want to join/be part of the pool so remove them from it
        ...
        //The user is requesting the operation on another user

        user = (await user).data();

        if (
          !pool.private ||
          (pool.allowShares && user.pools.includes(poolID)) ||
          pool.admins.includes(userID)
        ) {
          //If this user is allowed to perform this operation // TODO: check if the user requesting this operation is part of this pool

          if (join) {
            //If this user is allowing the specified user to join the pool
            if (targetUser.pendingPools.includes(poolID)) {
              //If the specified user has requested to join this pool
              ...
              //The user has not requested to join this pool so send them an invite and allow them to join
            ...
            //The user is requesting that the specifid uid not be allowed in the pool

            if (pool.admins.includes(userID)) {
              //If the user is an admin kick the requested user from the pool
              ...
              //The user is not an admin so send a request to the admin to kick the specified user
          ...
          //The user does not have permission for this operation so deny their request
          return {
      ...
      throw new functions.https.HttpsError(
        "failed-precondition",
        "Unable to join or invite to pool",
        data
      );
    }
  } else {
...
exports.setPoolNotificationPreference = functions.https.onCall(async function (
  data,
  context
) {
  /*
...
exports.addMessage = functions.https.onCall(async function (data, context) {
  ...
  if (data.poolID == null || data.text == null) {
    ...
    throw new functions.https.HttpsError(
      "failed-precondition",
      "The function was called with invalid Data: ",
      data
    );
  }
  ...
  if (true) {
    //// TODO: check to see if the user is able to send this chat message eg they are a part of the pool

    await db.collection("pools").doc(poolID).collection("messages").add({
    ...
    for (var i = 0; i < pool.users.length; i++) {
      if (pool.users[i].uid != uid) {
        noteOps.push(
          sendNotification(pool.users[i].uid, {
            senderID: uid,
            poolID: poolID,
            id: poolID,
            link: "/chat/?id=" + poolID,
    ...
    //The user does not have the permission to send theis message
    return {
      uid: userID,
      operation: "denied",
...
exports.scheduledAnnouncement = functions.pubsub
  ...
  .onRun(async function (context) {
    ...
    for (let x = 0; x < querySnapshot.docs.length; x++) {
      ...
      for (var i = 0; i < userDocs.length; i++) {
        if (
          !test ||
          userDocs[i].id == "YAk7ErsK3VTx7z6eLZUBECOPY9O2" ||
          userDocs[i].id == "9jFl5rEDLSWaEb50dZljVy1BVOr1" ||
          userDocs[i].id == "hmv13BjWz6gMYJ06jYMoO6zKyYt2" ||
          userDocs[i].id == "kvkL4oRtkDflKTeoOMJAkn2nRZe2"
        ) {
          promises.push(
            db
              .collection("users")
...
async function sendAnnouncementFunction(
  title,
  body,
  link,
  announcementId,
  ...
  if (test != null && test) {
    ...
    message.condition =
      "('user-9jFl5rEDLSWaEb50dZljVy1BVOr1' in topics)||('user-hmv13BjWz6gMYJ06jYMoO6zKyYt2' in topics)||('user-kvkL4oRtkDflKTeoOMJAkn2nRZe2' in topics)";
  } else {
...
exports.sendAnnouncement = functions.https.onCall(async function (
  data,
  context
) {
  ...
  if (admin.exists && admin.data().adminPermissions.announcements) {
    if (test) {
      console.log(
        "Sending a test announcment, title: " +
          title +
          ", body: " +
          body +
          ", link: " +
...
async function sendNotification(uid, message) {
  ...
  if (user != null && user.exists && user.data() != null) {
    ...
    let exludedNotifications = user.exludedNotifications
      ? user.exludedNotifications
      : [];

    if (exludedNotifications.includes(message.type)) {
      //If the user has requested to not receve these types of notifications the dont send anything
...
async function getUser(uidToGet) {
  //Gets a user and runs checks for errors and invalid pareameters
...
exports.freindRequest = functions.https.onCall(async function (data, context) {
  ...
  var pendingFreinds = doc.data().pendingFriends
    ? doc.data().pendingFriends
    : [];

  if (pendingFreinds.includes(userID)) {
    //If the other user has requested to be freinds

    //Remove the frind request notification that was sent to this user
    removeNotification(userID, "friend-request-" + uid);

    if (accepted) {
      //If both users have requested to be freinds set them as freinds in each of their docs. (Other user has sent a freind request and this user has rejected it)
      ...
      //Reject frind request (Other user has sent a request and this user has rejected it)
    ...
    //The other user has not requested to be freinds
    removeNotification(userID, "friend-accept-" + userID);

    if (accepted) {
      //User wishes to be freinds with uid so send freind request
      ...
      //User does not want to be freinds anymore (eg remove freind)
...
exports.banUser = functions.https.onCall(async function (data, context) {
  ...
  if (adminObject.exists && uidToBan != null) {
    if (!liftBan) {
      ...
      let bannedUserData = (
        await db.collection("bannedUsers").doc(uidToBan).get()
      ).data();
    ...
    throw new functions.https.HttpsError(
      "failed-precondition",
      "The function must be called " + "as an admin with valid data."
    );
  }
});
//:)
