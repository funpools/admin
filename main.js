...
// Setup for the new tiebreaker question button
$$("#t-question").on("click", function () {
  //store the question number for later use
  var questionNumb = makeid(10);
  ...
  db.collection("admins")
    ...
    .then(function (userData) {
      ...
      if (userData.exists) {
        ...
        if (
          self.app.views.main.router.currentRoute.path != "/" &&
          self.app.views.main.router.currentRoute.path !=
            "/fun-sports-pools-admin/"
        ) {
        ...
        async function devCode() {
          ...
          let devTagQuerySnapshot = await db
            .collection("pools")
          ...
          for (var i = 0; i < devTagQuerySnapshot.docs.length; i++) {
            ...
            for (let x = 0; x < data.questions.length; x++) {
              if (
                !resultQuestions.questions.includes(
                  data.questions[x].description
                )
              ) {
            ...
            for (let x = 0; x < data.tiebreakers.length; x++) {
              if (
                !resultQuestions.tiebreakers.includes(
                  data.tiebreakers[x].description
                )
              ) {
                resultQuestions.tiebreakers.push(
                  data.tiebreakers[x].description
                );
              }
            }
            ...
            getUser(doc.get("sender"), function (user) {
              ...
              $$("#" + doc.get("type")).append(
                '<li class="fb-' +
                  ...
                  messageIndex +
                  '\')" class="item-link item-content"><div class="item-inner"><div class="item-title-row">' +
  ...
  switch (message.type) {
    ...
    default:
      ...
      var popup = app.popup.create({
        content:
          '<div class="popup">' +
          '<div style="padding: 16px; padding-bottom: 0"><div class="float-right" style="margin-left: 16px"><a href="#" class="button button-outline popup-close">Close</a><a href="#" class="button button-fill resolve-' +
          message.id +
          '" style="margin-top: 8px">Resolve</a></div>' +
          '<div class="row justify-content-space-between align-items-center">' +
          '<div><p class="no-margin"><strong>From: </strong>' +
          message.sender.username +
          "<br><strong>Subject: </strong>" +
          message.subject +
          "<br><strong>Reply-To: </strong>" +
          mailto +
          "</p></div>" +
          '<div><p style="opacity: .5; margin: 0 0 4px 0">' +
          message.date +
          '</p><div class="display-flex justify-content-flex-end"><div class="picture" style="background-image: url(\'' +
          message.sender.picURL +
          "')\"></div></div></div>" +
  ...
  <div class="item-inner">\
    <div style="width: 100%">\
      ...
      <div class="item-input-wrap">\
        ...
        '" class="' +
        questionID +
        '-answer" type="text" placeholder="Your answer">\
      </div>\
    </div>\
    <div class="item-after">\
    <button id="' +
        answerID +
        '-deleteanswer" class="button delete-button" onclick="deleteAnswer(this)">Delete</button>\
      <button id="' +
        answerID +
        '-setanswer" class="button" >Correct</button>\
    </div>\
  </div>\
  </li>'
    );
...
function addQuestion(questionID, description, answers) {
  //Adds a multiple choice question to the html // NOTE: This only adds the question to the html not the database
  ...
  $$("#pool-questions").append(
    "<div id=" +
      questionID +
      ' class="question mc-question list no-hairlines no-hairlines-between">\
  <ul>\
    <li class="item-content item-input">\
      <div class="item-inner">\
      <div style="width: 100%">\
      ...
      '" type="text" placeholder="Your question">\
        ...
        <div class="item-after">\
          <button class="button delete-button" onclick="deleteQuestion(\'' +
      questionID +
      '\')">Delete</button>\
        </div>\
      </div>\
  ...
  document.getElementById("question-description-" + questionID).value =
    description;
...
// This is adds a numeric question to the html // NOTE: This only adds the question to the html not the database
function addNumericQuestion(questionID, description, answer) {
  // TODO: Check to see if this questionId has been added already if so dont add the question
  numericQuestionIDs.push(questionID);

  // Add the question html to the pool questions element
  $$("#pool-questions").append(
    '<div  id="' +
      questionID +
      '"  class="question n-question list no-hairlines no-hairlines-between">\
    <ul>\
      <li class="item-content item-input">\
      <div class="item-inner">\
      <div style="width: 100%">\
      ...
      <li class="item-content item-input">\
        <div class="item-inner"><div style="width: 100%">\
          ...
          <div class="item-input-wrap">\
            <input id="' +
      questionID +
      '-numeric-answer" class="" type="number">\
          </div>\
        </div></div>\
      </li>\
    </ul>\
...
let tiebreakerIDs = [];
// This is adds a tiebreaker question to the html // NOTE: This only adds the question to the html not the database
function addTiebreakerQuestion(questionID, description, answer) {
  // TODO: Check to see if this questionId has been added already if so dont add the question
  ...
  $$("#pool-questions").append(
    ...
    <ul>\
      ...
      <div class="item-inner">\
      <div style="width: 100%">\
        <div class="item-title item-label">Tiebreaker Question</div>\
        <div class="item-input-wrap">\
          <input id ="question-description-' +
      questionID +
      '" type="text" placeholder="Your question">\
...
function setAnswer(questionID, answerID) {
  ...
  if (correctAnswers[questionID] == answerID) {
    //If this is already marked as the correct answer(eg. the admin has pressed undo in the panel) then remove it from the correct answers
    app.dialog.confirm(
      ...
      function () {
        ...
        $$("#" + correctAnswers[questionID] + "-setanswer")
          .parent()
          .parent()
          .parent()
          .removeAttr("style");
    ...
    //Else add the answer to the list of correct answers and set its html to the correct color and state
...
function newAnnouncement() {
  ...
  // Send button setup
  $$(".announcement-popup")
    .find("#send-announcement")
    .off("click")
    .click(async function () {
      let sendDate = announcementDateInput.getValue()[0];
      let link = "";
      if (
        $$("#pool-select-dropdown").val() != null &&
        $$("#pool-select-dropdown").val() != ""
      ) {
...
$$(".new-tag-button").click(function () {
  let tagEl = $$(
    '<li class="tag-input"><div class="item-content"><div class="item-media"><a href="#" class="delete-tag-button"><i class="material-icons icon">cancel</i></a></div><div class="item-inner">' +
...
async function loadTags() {
  //Clears the current tags then loads them from the server
  ...
  universalData.tags.forEach((tag, i) => {
    ...
    if (tag.hidden === undefined || tag.hidden == false) {
      let tagEl = $$(
        '<li class="tag-input"><div class="item-content"><div class="item-media"><a href="#" class="delete-tag-button"><i class="material-icons icon">cancel</i></a></div><div class="item-inner">' +
          '<div class="item-title"><input class="tag-title-input" value="' +
          tag.title +
          '" type="text" name="Title" placeholder="Title" readonly></div>' +
          '</div></div><div class="sortable-handler"></div></li>'
      );
      ...
      let tagEl = $$(
        '<li class="tag-input"><div class="item-content"><div class="item-media"><a href="#" class="delete-tag-button"><i class="material-icons icon">cancel</i></a></div><div class="item-inner">' +
          ...
          tag.title +
          '" type="text" name="Title" placeholder="Title" readonly></div>' +
          "</div><a onClick=\"copyTagLinkToClipboard('" +
          tag.id +
          '\')">\
...
async function saveTags() {
  //Saves the tags and their order as currently displyed in the tags section
  app.preloader.show();
...
async function loadAdmins() {
  ...
  for (let i = 0; i < admins.length; i++) {
    ...
    $$("#id-" + adminID)
      .find("[type=checkbox]")
      .change(function () {
        //Save the admin anytime the check box changes
        saveAdmin(adminID, {
          adminPermissions: {
            users: $$("#id-" + adminID)
              .find(".users-checkbox")
              .is(":checked"),
            categories: $$("#id-" + adminID)
              .find(".categories-checkbox")
              .is(":checked"),
            announcements: $$("#id-" + adminID)
              .find(".announcements-checkbox")
              .is(":checked"),
            analytics: $$("#id-" + adminID)
              .find(".analytics-checkbox")
              .is(":checked"),
            developer: $$("#id-" + adminID)
              .find(".developer-checkbox")
              .is(":checked"),
          },
        });
...
async function saveAdmin(adminID, adminObj) {
  //Saves the admin then reloads all admins
...
async function deleteAdmin(adminID) {
  ...
  console.log(
    "Deleting admins from this panel is currently not finished please consult the devs."
  );
  return 1;
}
...
function addTag(el) {
  ...
  if (el.value.includes(",")) {
    ...
    chip.innerHTML =
      '<div class="chip" onclick="$$(this).remove()"><div class="chip-label">' +
      tag +
      '</div><a href="#" class="chip-delete"></a></div>';
